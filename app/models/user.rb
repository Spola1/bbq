class User < ApplicationRecord
  ...
  def self.find_for_vkontakte_oauth(access_token)
    if user = User.where(url: access_token.info.urls.Vkontakte).first
      ...
      User.create!(provider: access_token.provider, url: access_token.info.urls.Vkontakte,
                   username: access_token.info.name, nickname: access_token.extra.raw_info.domain, email: access_token.extra.raw_info.domain + '<hh user=vk>.com', password: Devise.friendly_token[0, 20])
    end
  end
  ...
  def set_email
    self.email = "change@me#{rand(777)}.example" if email.blank?
  end
